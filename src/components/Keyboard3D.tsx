...
const SHIFTED_TO_KEY: Record<string, string> = Object.fromEntries(
  ROWS.flat().filter((key) => key.shift).map((key) => [key.shift!, key.id]),
);
...
export function keysForChar(char: string | undefined): string[] {
  ...
  else if (ROWS.some((row) => row.some((key) => key.id === char))) id = char;
  if (!id) return [];
...
export function Keyboard3D({ nextChar, active, combo = 0, className }: Keyboard3DProps) {
  ...
  return (
    <div className={cn("kb3d", className)} aria-hidden="true">
      <div ref={boardRef} className="kb3d-board" style={{ "--kb-glow": glow.toFixed(2) } as React.CSSProperties}>
